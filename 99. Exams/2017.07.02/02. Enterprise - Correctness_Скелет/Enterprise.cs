...
public class Enterprise : IEnterprise
{
    ...
    private Dictionary<HireDate,HashSet<Employee>> byHireDate;
    private Dictionary<Position,HashSet<Employee>> byPosition;
    private Dictionary<Salary,HashSet<Employee>> bySalary;
    private Dictionary<FirstName,HashSet<Employee>> byFirstName;

    public Enterprise ()
	{
        this.employees = new List<Employee>();
        this.byGuid = new Dictionary<Guid, Employee>();
        this.byHireDate = new Dictionary<HireDate, HashSet<Employee>>();
        this.byPosition = new Dictionary<Position, HashSet<Employee>>();
        this.bySalary = new Dictionary<Salary, HashSet<Employee>>();
        this.byFirstName = new Dictionary<FirstName, HashSet<Employee>>();
	}
        ...
        foreach (var date in this.byHireDate)
	    {
            ...
            if(timespan.Days > months * 30)
	        {
                foreach (var employee in date.Value)
	            {
                    ...
                    if (!this.bySalary.ContainsKey(employee.Salary))
	                {
                        this.bySalary.Add(employee.Salary,null);
	                }
                    this.bySalary[employee.Salary] = employee;
	            }
                isRaise = true;
	        }
	    }
