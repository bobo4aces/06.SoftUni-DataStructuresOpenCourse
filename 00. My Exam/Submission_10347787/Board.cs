...
public class Board : IBoard
{
    Comparison<Card> nameComparer =
        (first, other) =>
        {
            int name = Convert.ToInt32(first.Name[first.Name.Length - 1]);
            int nameOther = Convert.ToInt32(other.Name[other.Name.Length - 1]);
    ...
    public IEnumerable<Card> ListCardsByPrefix(string prefix)
    {
        ...
        foreach (var item in this.cards.Where(x => x.Name.StartsWith(prefix)))
        {
            cards.Add(item);
        }
